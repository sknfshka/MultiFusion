...
/*void RealPaintWidget::keyPressEvent( QKeyEvent * event )
{
    QWidget::keyPressEvent(event);

    int key = event->key();
    QMessageBox::warning(0,"check","check");
    if ( selection.getIsNewFigure() && key == Qt::Key_Return && currentTool != 0 && currentTool->createStyle() == FigureToolInterface::paintAfterRelease )
    {
        GObjectInterface *s = layers[currentLayer]->object( 0 );
        if( s != 0 )
        {
            QMessageBox::warning(0,"Trace1",QString::number(selection.getIsNewFigure()) + " "
                                 + QString::number(selection.getInputMode()));
            selection.setIsNewFigure(false);
            s->deleteLastGuide();
            s->cloneFrameToAll(currentFrame);
        }
